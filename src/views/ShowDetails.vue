<template>
  <div class="flex flex-col">
    ...
    >
      <img
        :src="'http://image.tmdb.org/t/p/w1280' + tv.poster_path"
        alt="poster"
        class="w-72 rounded-lg"
      />
      <div class="mt-2 md:mt-0 md:ml-20">
        <h1 class="text-white font-bold text-2xl">{{ tv.name }}</h1>
        ...
        <div class="mt-8">
          <a
            href="#"
            class="bg-indigo-400 hover:bg-indigo-600 text-black text-sm py-2 px-6 font-semibold rounded flex items-center w-36"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
...
export default {
  ...
  mounted() {
    ...
    axios.get(this.url + this.show_id + this.api_key).then((response) => {
    ...
    axios
      .get(this.url + this.show_id + "/credits" + this.api_key)
      .then((response) => {
    ...
    axios
      .get(this.url + this.show_id + "/similar" + this.api_key)
      .then((response) => {
